...
* Pulizia R@1nbow.
* Ora esistono solo 2 metodi per l'inject dello SLIC, il metodo ALTERNATIVE, è lo SLIC 2.3 Dell di DAZ.
* Sostituito il bootloader Zsmin 0.97 con un OpenLoader più recente, ora lo SLIC può essere sostituito manualmente.
* Fix Minori.
...
* Aggiunto tema per ipovedenti, è visualizzabile solo con temi di contrasto elevato.
...
/ForceMethod=
 If the version of Windows you'd run provides more than one method of activation, you can force the Re-Loader to run another method of activation.
...
/LoaderMode=
 It assumes values ??Default, Alternative, Forceful, Old (Ex: / LoaderMode = "Default"). You can use it for a different OEM activation method.
...
/BackupActivation
 BackupActivation Create a folder on your desktop that contains the backup of the activations of Microsoft products found.

/FindPID
 Find all the Windows Product Key and Office present on the local machine and performs a PID Checker.
...
======
Example use / Logo 1: Re-Loader.exe / Logo = "AutoDetect" (so the Loader between their libraries if a logo is compatible with the system)
Example use / Logo 2: Re-Loader.exe / Logo = "VMWare" (is forced to install the VMWare logo)
...
# 10 - Thanks
################################################## ########################
Thank: QAD, xinso, Hotbird64, nonosense, Nosferati87, Mikmik38, Cynecx, heldigard, Daz, Alphawaves, Nummer for various sources and work!
